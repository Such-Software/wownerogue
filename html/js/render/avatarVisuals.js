...
(function (root) {
    ...
    function drawIcon(ctx, visual, rect, opts) {
        ...
        var boxW = rect.w || rect.size || 32;
        var boxH = rect.h || rect.size || 32;
        var natural = Math.min(boxW, boxH);
        var legend = visual.context === 'legend' || opts.context === 'legend';
        var scale = opts.scale || (legend ? 0.78 : 1);
        var size = opts.size || Math.max(1, Math.round(natural * scale));
        var offsetX = opts.offsetX || 0;
        var offsetY = opts.offsetY != null ? opts.offsetY : (legend ? Math.round(natural * 0.08) : 0);
        var x = rect.x + Math.max(0, (boxW - size) / 2) + offsetX;
        var y = rect.y + Math.max(0, (boxH - size) / 2) + offsetY;
        ctx.clearRect(rect.x, rect.y, boxW, boxH);
